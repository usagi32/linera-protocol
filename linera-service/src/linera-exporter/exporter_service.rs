...
mod test {
    ...
    impl ProtocolState {
        ...
        // the idea is to produce a densly connected directed acyclic graph                                      ↓↙↙↙  ↘↓↙↙ ↘↘↓↙  ↘↘↘↓
        // blocks in chains with even index are directed linearly                                            B   1       2       3       4
        // same with even index also have parallel ancestors in previous chain with odd index                    ↓       ↓       ↓       ↓
        // blocks in chains with odd index are densly connected with every block in last even chain          C - 1   ->  2   ->  3   ->  4
